...
2. **X 公開ごとの URL 記帳** — 各メタデータファイルの「投稿後」行に記入。記帳済みは実例集（7/24）・台所（7/25）・常備品（7/26）。バックリンク 7/27 → ボタン 7/28 → what-is 7/29 → csv 7/30 → **基礎編 7/31（要予約）** と続く（基礎編以外は予約済みなので、公開後に URL を拾って記入するだけ）
