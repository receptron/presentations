...
|---|---|---|---|---|
| 1–2 | 0:00–0:14 | `broll-tabs.mp4` | B-roll: 通常ターミナルのタブ往復 | **UI モックで仮埋め（2026-08-05）** — html_tailwind アニメ（タブ切替・待ちの演出）。実写 B-roll はユーザーの ⌘⇧5 録画待ち |
| 3 | 0:14–0:22 | `grid-open.mp4` | グリッドがバッと開く決め画 | **撮影済み・埋め込み済み** — 白リード除去・描画流入から開始に切り直し（2026-08-06）。状態リングが薄い点は残っており、最終版は staging 直後に撮り直し推奨 |
...
| 8 | 1:08–1:18 | `worktree-pr.mp4` | worktree → diff → PR ボタン | **撮影済み・埋め込み済み（2026-08-06）** — ⎇ fix-login 行から起動 → +1 バッジ → Changes vs main パネル（Commit/Push/Open PR）。フォーム操作部は拡大インセット（worktree-zoom.mp4）を重ねて可読化。10テイク目。起動は worktree 行クリック経路が確実（create-and-launch 経路はセルが出ないことがある） |
...
ビート 1・2・6・7・9 は `UI mock — 実写差し替え予定` のタグ付き html_tailwind アニメで仮埋めしてある。
これは**ドラフトを通しで観られるようにするための仮素材**で、公開版に残す場合は「意匠化されたモック」として
明示的に承認を取ること — 生成 AI 動画（Veo 等）での偽実写は不採用（文字が崩れる・実写を装った UI は
ローンチ動画で最も信頼を失う嘘。生成画像を実スクショとして出してしまった前例 issue もある）。
