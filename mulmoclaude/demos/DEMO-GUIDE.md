...
- **固定幅カラムのテキストは折り返させない**: `w-XX`（`w-72` 等）でカラム幅を固定した中に日本語テキストを置くと、幅が足りず途中で改行される。ルール名・見出しなど 1 行で読ませたい要素には `whitespace-nowrap` を付け、幅は実文字数で見積もって余裕を持たせる（10 文字級の日本語見出しは `w-72`=288px では折り返す → `w-80` 前後＋`whitespace-nowrap`）。**折り返しは HTML では気づけないので、生成画像（`output/<stem>/images/<n>p.png`）を必ず目視で確認する**（2026-07 のまとめビート指摘: `w-72` の 3 ルール名が全て 2 行に折り返していた）
...
- **同一セッションを複数ブラウザタブで開いて「ビューを表示したまま」の絵を撮らない**: present（回答や presentCollection の表示）はブラウザタブごとではなくセッション単位の状態で、エージェントのターンが完了すると同じセッションを開いている全タブの右ペインが回答ドキュメントに差し替わる（実例: 別タブに表示しておいたダッシュボードが完了時に置き換わり、「開いたまま自動更新」の瞬間を撮り損ねた）。その絵が欲しいときは、完了後にビューを開き直して before/after を別々に撮り、スライド側のクロスフェードで合成する
...
- 尺を字数から見積もるときは**約 6 字/秒**で計算する（上の 1,050 字 ≈ 3 分と同源。Gemini Kore ja のビート別実測 5.8〜6.5 字/秒）。低く見積もると尺不足になり、ナレーション増補と TTS・retime のやり直しが発生する（実例: 60 秒版を 4.8 字/秒で見積もり、45.7 秒しかなく作り直した）
- 内部動作の説明（仕様目線）ではなくユーザー便益で語る。例: 「済むと次回分が自動で生まれる」→「毎月の用事も、忘れない」
- 固有名詞の TTS 安定化: 「MulmoClaude」はナレーションでは「マルモクロード」と表記する
- TTS が読み間違える語はナレーション側だけ言い換える(スライド表記は変えない)。例: 「表」は「おもて」と読まれるため「一覧表」(いちらんひょう)にする / 「空のまま」は「そらのまま」と読まれるため「からっぽのまま」にする
...
上の項目が「どう作るか」の手順、この節が「どう訳すか」の中身。**vision デッキも含めた全デッキに適用する**。日本語版がレビュー済みの正で、英語版はそこからの翻訳。

**語彙**

- 「育てる」（人が主語・他動詞）は **nurture**。`raise` は使わない。正は vision の `the-assistant-you-nurture`（同デッキで nurture を一貫使用）
- 「育つ」（コレクション側が主語・自動詞）は **grow**。例:「会話で、育つ」→ `They grow by talking` /「使うほど育っていく」→ `grow the more you use them`
- 同じ `raise` でも「（生産性を）上げる」は別語なので置換しない。例: `Nurturing an assistant raises your own productivity`
- 固有の対訳は 1 対 1 に固定し、デッキ内で言い換えない（recipe book / shopping list / ledger / collection）
- 相互参照の動画タイトルは `PROGRESS.md` のタイトル管理表から取る（その場で訳し下ろさない）

**意味を動かさない**

- 日本語にない主張を足さない。とくに「過去の記録を参照した」「好みを学習した」等、そのビートまでに紹介していない能力を先出ししない
- 依頼文（ユーザーの発話）から条件を落とさない。**返答が言及する条件は、必ず依頼の側にも残す**。実例: collections-showcase beat 3 で依頼から「木曜は洋風」が落ち、アシスタントの返答だけが木曜に言及していた（頼まれていないことに答えている状態）
- 削るなら、そのビートの主眼から外れる情報を削る。順序を入れ替えて主眼をずらさない

**数値と画面**

- 数値は**その言語のキャプチャに合わせる**。ja と en で件数が違ってよい（ja「18品」/ en `Nineteen items`）。禁止なのは、同じスライドに写っている数字と本文が食い違うこと
- 英語では数を綴り字にしてよい（`two kinds` / `3 p.m.` / `three cities`）。機械検査で「数字が片方にしかない」と出ても、たいていはこれによる誤検出

**トーンと文体**

- ja の「です・ます」の落ち着きを保つ。煽り・命令形・マーケティング語を持ち込まない
- 英語圏のスラングや作品由来の言い回しを避ける（`make it so` は SF 的で浮く → `I'd like it this way`）
- 見出しは ja と同じ長さ・粒度にする。片方だけ説明過多にしない
- 引用は ja「」→ en `"..."`。UI 要素名は en では引用符なしでよい（`Click the Collections button`）

**翻訳後の検査**

1. ja / en を beat ごとに並べ、**全ビートのナレーションとスライド可視テキストを読む**
2. 機械的に当たりを付ける: 算用数字の集合・引用の数・ナレーション長の比（デッキ内の中央値から外れるビートは情報の欠落 / 追加を疑う）
3. 引っかかった箇所は**実キャプチャを開いて確認する**（画面が正、本文が従）
4. 第三者（Codex 等）にレビューさせるときは、ja/en 対照・キャプチャ・この節のルールを一緒に渡す。渡さないと「言語ごとにデータが違う」ことを知らずに数値の誤指摘が出る
