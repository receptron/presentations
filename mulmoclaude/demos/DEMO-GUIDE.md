...
`mulmoclaude/demos/` 配下のデモ（What/How コンテンツ）を量産するためのパターン集。
正とする実例はレビュー承認済みの `collection-creation-demo.json`。新しいデモを書くときは、このガイドと実例の両方を参照する。

## ビート構成の型

| 順 | ビート | 役割 |
|---|---|---|
| 1 | 課題提示 + 結果先出し | 「〜、大変じゃない？」という共感 + 完成形のスクショを最初に見せる（前半離脱の防止） |
...
| n-1 | まとめ | Step 一覧を行形式で。各行はビートの STEP バッジと同じ番号・同じ語 |
