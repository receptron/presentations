...
- ペース: 動画告知は **1 日 1 本・20:30 JST**（同一話題の同日連投は減衰するため 1 日 1 本を守る。20:30 は既存運用のリリース速報（テキスト）と同じ夜枠 — 速報と同日になる場合は動画側を昼 12:15 にずらす）。朝 10:00 は 10 秒クリップ枠（過去の未紹介機能の消化用・週 3 上限）で、話題が別なら同日でも可。レーン全体の構成は `../content-roadmap.md` の「リリース告知の 3 レーン構成」が正本
